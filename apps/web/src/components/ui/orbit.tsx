  ...
  return (
    <div className="relative w-[420px] aspect-square mx-auto" ref={orbitRef}>
      ...
      <motion.div
        ...
        className="absolute inset-0 rounded-full border bg-neutral-900/5 backdrop-blur-xl border-white/5 [box-shadow:0_-20px_80px_-20px_#ffffff1f_inset] "
      ></motion.div>
      <motion.div
        initial={{ y: 40, opacity: 0 }}
        animate={{ y: 0, opacity: 1 }}
        transition={{ duration: 0.3, ease: "easeOut", delay: 0.1 }}
        className="absolute inset-[45px] rounded-full border border-white/5  backdrop-blur-xl [box-shadow:0_-20px_80px_-20px_#ffffff1f_inset]"
      ></motion.div>
      <motion.div
        initial={{ y: 40, opacity: 0 }}
        animate={{ y: 0, opacity: 1 }}
        transition={{ duration: 0.3, ease: "easeOut" }}
        className="absolute inset-[90px] rounded-full border border-white/5  backdrop-blur-xl [box-shadow:0_-20px_80px_-20px_#ffffff1f_inset] "
      ></motion.div>
      <motion.div
        initial={{ y: 40, opacity: 0 }}
        animate={{ y: 0, opacity: 1 }}
